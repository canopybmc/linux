...
static int umac_init_ring_descriptors(struct net_device *ndev)
{
	struct umac_priv *umac = netdev_priv(ndev);
...
static int umac_phy_fixup(struct phy_device *phy_dev)
{
	...
	phy_write(phy_dev, PHY_88E1514_PAGE_ADDRESS, 0);
...
static netdev_tx_t umac_start_xmit(struct sk_buff *skb, struct net_device *ndev)
{
	struct umac_priv *umac = netdev_priv(ndev);
	struct umac_tx_desc_entry *ptxdesc;
	u8 *pframe;
...
static int umac_open(struct net_device *ndev)
{
	...
	umac_init_ring_descriptors(ndev);
	umac_init_hw(ndev);
...
static int umac_init_mac_address(struct net_device *ndev)
{
	...
	if (ret || !is_valid_ether_addr(addr)) {
		...
		netdev_info(ndev, "Read MAC address %pM from DTB\n",
			    ndev->dev_addr);
	}

	umac_set_mac_address(ndev, ndev->dev_addr);
...
static int umac_setup_phy(struct net_device *ndev)
{
	...
	if (of_phy_is_fixed_link(phy_handle)) {
		struct device_node *fixed_link_node =
			of_get_child_by_name(phy_handle, "fixed-link");
		...
		umac->phy_dev->duplex = of_property_read_bool(fixed_link_node,
							      "full-duplex");
		umac->phy_dev->pause = of_property_read_bool(fixed_link_node,
							     "pause");
		umac->phy_dev->asym_pause =
			of_property_read_bool(fixed_link_node, "asym-pause");
